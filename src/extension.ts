...
export function activate(context: vscode.ExtensionContext) {
    console.log('Rails OpenAPI Gen extension is now active!');

    const diagnosticCollection = vscode.languages.createDiagnosticCollection('rails-openapi-gen');
    context.subscriptions.push(diagnosticCollection);

    context.subscriptions.push(
        vscode.commands.registerCommand('railsOpenapiGen.generate', async () => {
            await runRailsCommand('openapi:generate');
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('railsOpenapiGen.check', async () => {
            await runRailsCommand('openapi:check');
        })
    );
                ...
                provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                    ...
                    if (lineText.match(/#\s*@openapi\s*$/)) {
                        ...
                        jsonFields.forEach(fieldName => {
                            ...
                            item.insertText = new vscode.SnippetString(`${fieldName}:\${1|integer,string,boolean,number,array,object|}`);
                    ...
                    if (lineText.match(/\w+:\w+/)) {
                        ...
                        const requiredItem = new vscode.CompletionItem('required:', vscode.CompletionItemKind.Property);
                        ...
                        const exampleItem = new vscode.CompletionItem('example:', vscode.CompletionItemKind.Property);
                        ...
                        const formatItem = new vscode.CompletionItem('format:', vscode.CompletionItemKind.Property);
        ...
        vscode.languages.registerCompletionItemProvider(
            ['ruby', 'erb'],
            {
                provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                    ...
                    // @openapi snippet
                    const openapiItem = new vscode.CompletionItem('@openapi', vscode.CompletionItemKind.Snippet);
                    ...
                    const minimalItem = new vscode.CompletionItem('@openapi (minimal)', vscode.CompletionItemKind.Snippet);
                    ...
                    if (inBlockComment) {
                        const operationItem = new vscode.CompletionItem('@openapi_operation', vscode.CompletionItemKind.Snippet);
        ...
        vscode.languages.registerCompletionItemProvider(
            ['ruby', 'erb'],
            {
                provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                    ...
                    const attributes = [
                        { name: 'summary:', snippet: 'summary:"${1:Summary}"', doc: 'Brief summary of the operation' },
                        { name: 'tags:', snippet: 'tags:[${1:Tag1,Tag2}]', doc: 'Operation tags for grouping' },
                        { name: 'description:', snippet: 'description:"${1:Description}"', doc: 'Detailed operation description' },
                        { name: 'response_description:', snippet: 'response_description:"${1:Response}"', doc: 'Description of the response' },
                        { name: 'parameters:', snippet: 'parameters:"${1:param1, param2}"', doc: 'Operation parameters' },
                        { name: 'requestBody:', snippet: 'requestBody:"${1:Request body}"', doc: 'Request body description' },
                        { name: 'responses:', snippet: 'responses:"${1:Response codes}"', doc: 'Response codes and descriptions' }
                    ];
        ...
        vscode.languages.registerHoverProvider(['ruby', 'erb'], {
            provideHover(document, position) {
                ...
                if (openApiMatch) {
                    ...
                    markdown.appendCodeblock('# @openapi id:integer\n# @openapi name:string required:true description:"User name"', 'ruby');
...
function extractJsonFieldsFromDocument(document: vscode.TextDocument): string[] {
    ...
    const jbuilderMethods = ['array', 'set!', 'merge!', 'extract!', 'cache!', 'cache_if!', 'cache_root!', 'ignore_nil!'];
...
function isJbuilderFile(document: vscode.TextDocument): boolean {
    return document.fileName.endsWith('.jbuilder') || 
           document.fileName.endsWith('.json.jbuilder');
}
...
async function runRailsCommand(command: string) {
